...
export function createMewLockDepositTx(
	...
	const mewLockBox = new OutputBuilder(lockBoxValue, MEWLOCK_CONTRACT_ADDRESS)
...
export async function createMewLockWithdrawalTx(
	...
	try {
		...
		const ergFee = normalizedLockBox.value >= BigInt(MIN_ERG_FEE_THRESHOLD) 
		...
		const tokenFees = normalizedLockBox.assets.map(token => {
			const tokenAmount = BigInt(token.amount);
			const feeAmount = tokenAmount >= BigInt(MIN_TOKEN_FEE_THRESHOLD)
